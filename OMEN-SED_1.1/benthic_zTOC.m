classdef benthic_zTOC < handle
    ...
    methods
        ...
        function res = calc(obj, bsd, swi, res)
            ...
            rTOC.a1=(bsd.w-sqrt(bsd.w.^2+4.*obj.DC1.*obj.k))./(2.*obj.DC1);
            rTOC.b1=(bsd.w+sqrt(bsd.w.^2+4.*obj.DC1.*obj.k))./(2.*obj.DC1);
            rTOC.a2=(-obj.k./bsd.w);
            rTOC.A1 =-(swi.C0i.*rTOC.b1.*exp(rTOC.b1.*bsd.zbio))./(rTOC.a1.*exp(rTOC.a1.*bsd.zbio)-rTOC.b1.*exp(rTOC.b1.*bsd.zbio)+bsd.tol_const);
            rTOC.A2 =(rTOC.A1.*(exp(rTOC.a1.*bsd.zbio)-exp(rTOC.b1.*bsd.zbio))+swi.C0i.*exp(rTOC.b1.*bsd.zbio))./(exp(rTOC.a2.*bsd.zbio)+bsd.tol_const);
            Names = fieldnames(rTOC);
            for i = 1:size(Names,1)
                %                 if sum(isnan(rTOC.(Names{i}))==1) > 0 %
                if any(~isfinite(rTOC.(Names{i}))) || any(~isreal(rTOC.(Names{i})))
                    warning(['Sum of rTOC.', Names{i} ,' Fractions contains NaNs!!!'])
                    rTOC.(Names{i})(isnan(rTOC.(Names{i}))==1)=NaN;
                end
            end
            clear Names
            ...
            res.F_TOC1=-(1-bsd.por).*bsd.w.*rTOC.A2.*exp(rTOC.a2.*bsd.zinf);
            ...
            if(false)   % prints for checking TOC integration for SWI-flux test
                Int_nonbio = obj.k.*((rTOC.A2/rTOC.a2*exp(rTOC.a2*bsd.zinf)) - rTOC.A2/rTOC.a2*exp(rTOC.a2*bsd.zbio));
                Int_nonbio = sum(Int_nonbio)
                
                Int_bio_Maple = obj.k.*( (rTOC.A1*(exp(rTOC.a1*bsd.zbio)/rTOC.a1 - exp(rTOC.b1*bsd.zbio)/rTOC.b1)) ...
                    + swi.C0i./rTOC.b1 .* exp(rTOC.b1*bsd.zbio)...
                    - (rTOC.A1.*(1./rTOC.a1 - 1./rTOC.b1)+ swi.C0i./rTOC.b1)); % for C(inf) = 0:   k* (A1/a1*exp(a1*zinf)-A1/a1)
                Int_bio_Maple = sum(Int_bio_Maple)
                
                Int_bio_z0 = obj.k.*((rTOC.A1.*(1./rTOC.a1 - 1./rTOC.b1)+ swi.C0i./rTOC.b1));
                Int_bio_z0 = sum(Int_bio_z0)
                Int_bio_zbio = obj.k.*((rTOC.A1*(exp(rTOC.a1*bsd.zbio)/rTOC.a1 - exp(rTOC.b1*bsd.zbio)/rTOC.b1)) + swi.C0i/rTOC.b1 * exp(rTOC.b1*bsd.zbio));
                Int_bio_zbio = sum(Int_bio_zbio)
                Int_nonbio_zbio = obj.k.*(rTOC.A2/rTOC.a2*exp(rTOC.a2*bsd.zbio));
                Int_nonbio_zbio = sum(Int_nonbio_zbio)
                Int_nonbio_zinf = obj.k.*((rTOC.A2/rTOC.a2*exp(rTOC.a2*bsd.zinf)));
                ...
                for i=1:1:1001
                    z(i) = (i-1)/10;
                    if(z<=bsd.zbio)
                        C1(i)=sum(rTOC.A1.*(exp(rTOC.a1.*z(i))-exp(rTOC.b1.*z(i)))+swi.C0i.*exp(rTOC.b1.*z(i)),2);
                    else
                        C1(i)=sum(rTOC.A2.*exp(rTOC.a2.*z(i)),2);
                    end
                end
                figure
                plot(C1,-z)
        ...
        function [C, C1] = calcC(obj, z, bsd, swi, r)
            ...
            rTOC = r.rTOC;
            if(z<=bsd.zbio)
                C1=rTOC.A1.*(exp(rTOC.a1.*z)-exp(rTOC.b1.*z))+swi.C0i.*exp(rTOC.b1.*z);
                % %                 C2=rTOC.A12.*(exp(rTOC.a12.*z)-exp(rTOC.b12.*z))+swi.C02.*exp(rTOC.b12.*z);
                
            else
                
                C1=rTOC.A2.*exp(rTOC.a2.*z);
        ...
        function [Cflx, C1flx] = calcCflx(obj, z, bsd, swi, r)
            ...
            if(z<bsd.zbio)
                ...
                dC1dz =  rTOC.A1.*(rTOC.a1.*exp(rTOC.a1.*z)-rTOC.b1.*exp(rTOC.b1.*z))+swi.C0i.*rTOC.b1.*exp(rTOC.b1.*z);
