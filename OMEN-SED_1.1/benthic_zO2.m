classdef benthic_zO2
    ...
    methods
        ...
        function r = calc(obj, bsd, swi, r)
            ...
            f1=@(zox) obj.calcbc(zox,bsd,swi,r,1); %% This seems to be the source of error !!!!!!!!!
            funx1 = f1(1e-10);
            f2=@(zox) obj.calcFO2(zox,bsd, swi, r);
            funx2 = f2(1e-10);
%                         if ~isreal(fun0)~=1;disp('fun0=inf');end
            ...
            if bsd.usescalarcode
                ...
                elseif conczinf >=0    	% still O2 at zinf -> zox = zinf
                    ...
                    bctype = 1;         % BC: zero concentration
                    r.zox = fzero(fun,[1e-10 bsd.zinf],bsd.fzerooptions); % [r.zox, fval, exitflag, output] =
                    conczinf = 0.0;
                end
        ...
        function [flxzox, conczox, flxswi, r] = calcbc(obj, zox, bsd, swi, r, bctype)
            ...
            rO2.ls = r.zTOC.prepfg_l12(bsd, swi, r, obj.reac1, 0, bsd.z0, zox, obj.DO21, obj.DO22);
            
            % basis functions at upper boundary
            [ e_0, dedz_0, f_0, dfdz_0, g_0, dgdz_0] ...
                = r.zTOC.calcfg_l12(bsd.z0, bsd, swi, r, obj.reac1, 0, rO2.ls);
            % ... and lower boundary
            [ e_zox, dedz_zox, f_zox, dfdz_zox, g_zox, dgdz_zox] ...
                = r.zTOC.calcfg_l12(zox, bsd, swi, r, obj.reac1, 0, rO2.ls);
        ...
        function FO2 = calcFO2(obj, zox, bsd, swi, r)
            ...
            FO2 = zox./(bsd.zoxgf + zox).*r.zTOC.calcReac(zox, bsd.zinf, tmpreac1, bsd, swi, r);
        ...
        function FO2 = calcFO2_exact(obj, zox, bsd, swi, r)
            ...
            FO2 = zox./(bsd.zoxgf + zox).*(r.zTOC.calcReac(r.zno3, bsd.zinf, tmpreac1_N, bsd, swi, r) ...
                + r.zTOC.calcReac(r.zno3, r.zso4, tmpreac1_S, bsd, swi, r));
            % NB (1-bsd.por)/bsd.por  has been included in OC etc stoich factors, so this is flux / cm^2 pore area
        ...
        function [O2, flxO2, flxO2D, flxO2adv] = calcO2(obj, z, bsd, swi, r)
            ...
            if z <= r.zox    % <= so handle case of fully oxic with zox = zinf
                ...
                [ e, dedz, f, dfdz, g, dgdz] ...
                    = r.zTOC.calcfg_l12(z, bsd, swi, r, obj.reac1, 0, r.rO2.ls);
